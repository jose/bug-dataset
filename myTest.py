...
def read_config():
    """
    Read the .bugsjs.config in the checkout directory into a dictionary.  The
    config files contains information of the bug, e.g.,

    # File automatically generated by BugsJS
    project=Bower
    bug-ID=1
    version=fixed
    """
    param_dict = {}
    with open('.bugsjs.config') as f:
        for line in f:
...
def test():
    ...
    sp.call("rm -rf test_results.json test_results.json.zip coverage/", shell=True)
