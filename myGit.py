...
def checkout(param_dict):
    ...
    # Create a .bugsjs.config file in the param_dict["output"] directory with
    # information of the checkout project/version, so that other/subsequent commands
    # could use that information to perform other tasks
    f = open('.bugsjs.config', 'w')
    f.write('# File automatically generated by BugsJS\n')
    f.write('project=' + str(param_dict["project"]) + '\n')
    f.write('bug-ID='  + str(param_dict["bug-ID"]) + '\n')
    f.write('version=' + str(param_dict["version"]) + '\n')
    f.close()
